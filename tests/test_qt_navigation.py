...
class FakeNavigationBatchService:
    def read_csv(self, path):
        raise AssertionError("navigation test must not read CSV")

    def parse_pass_rows(self, rows, defaults):
        raise AssertionError("navigation test must not parse CSV")

    def parse_badge_rows(self, rows, folder, defaults):
        raise AssertionError("navigation test must not parse CSV")

    def review(self, kind, items):
        raise AssertionError("navigation test must not review CSV")

    def export_template(self, kind, path):
        raise AssertionError("navigation test must not export template")
...
def test_main_viewmodel_accepts_batch_route(qtbot):
    vm = MainViewModel()
    with qtbot.waitSignal(vm.route_changed, timeout=1000) as signal:
        vm.set_route("batch")
    assert signal.args == ["batch"]
    assert vm.active_route == "batch"
...
def test_dashboard_batch_action_navigates_to_batch(qapp, qtbot):
    window = _window(qapp)
    qtbot.addWidget(window)
    qtbot.mouseClick(window.dashboard.batch_button, Qt.MouseButton.LeftButton)
    assert window.active_route == "batch"
    assert window.stack.currentWidget() is window.batch_page
